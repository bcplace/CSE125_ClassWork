/* Generated by Yosys 0.25 (git sha1 e02b7f64bc7, clang 14.0.0 -fPIC -Os) */

(* hdlname = "\\graycounter" *)
(* dynports =  1  *)
(* top =  1  *)
(* src = "graycounter.sv:1.1-35.10" *)
module graycounter(clk_i, reset_i, up_i, gray_o);
  wire _00_;
  wire _01_;
  wire _02_;
  wire _03_;
  wire _04_;
  wire _05_;
  wire _06_;
  (* src = "graycounter.sv:12.24-12.32" *)
  (* unused_bits = "0 1 2" *)
  wire [3:0] binary_l;
  (* src = "graycounter.sv:3.17-3.22" *)
  input clk_i;
  wire clk_i;
  (* src = "graycounter.sv:13.24-13.33" *)
  wire [3:0] counter_n;
  (* src = "graycounter.sv:6.26-6.32" *)
  output [3:0] gray_o;
  wire [3:0] gray_o;
  (* src = "graycounter.sv:8.24-8.30" *)
  reg [3:0] gray_r;
  (* src = "graycounter.sv:4.17-4.24" *)
  input reset_i;
  wire reset_i;
  (* src = "graycounter.sv:5.17-5.21" *)
  input up_i;
  wire up_i;
  assign _00_ = ~(gray_r[2] ^ gray_r[3]);
  assign _01_ = ~(gray_r[1] ^ gray_r[2]);
  assign _02_ = ~(gray_r[0] ^ gray_r[1]);
  assign _03_ = _02_ | ~(up_i);
  assign _04_ = _03_ | _01_;
  assign counter_n[2] = _04_ ^ _00_;
  assign counter_n[3] = _04_ ? gray_r[3] : gray_r[2];
  assign _05_ = ~(_02_ ^ up_i);
  assign counter_n[0] = _05_ ^ counter_n[2];
  assign _06_ = _03_ ^ _01_;
  assign counter_n[1] = _06_ ^ counter_n[3];
  (* \always_ff  = 32'd1 *)
  (* src = "graycounter.sv:27.4-33.7" *)
  always @(posedge clk_i)
    if (reset_i) gray_r[0] <= 1'h0;
    else gray_r[0] <= counter_n[0];
  (* \always_ff  = 32'd1 *)
  (* src = "graycounter.sv:27.4-33.7" *)
  always @(posedge clk_i)
    if (reset_i) gray_r[1] <= 1'h0;
    else gray_r[1] <= counter_n[1];
  (* \always_ff  = 32'd1 *)
  (* src = "graycounter.sv:27.4-33.7" *)
  always @(posedge clk_i)
    if (reset_i) gray_r[2] <= 1'h0;
    else gray_r[2] <= counter_n[2];
  (* \always_ff  = 32'd1 *)
  (* src = "graycounter.sv:27.4-33.7" *)
  always @(posedge clk_i)
    if (reset_i) gray_r[3] <= 1'h0;
    else gray_r[3] <= counter_n[3];
  assign binary_l[3] = gray_r[3];
  assign gray_o = gray_r;
endmodule
